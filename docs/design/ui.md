...
```
+-------------------------------------------+
| ノートのタイトル ................ 🕐 21d  |   ← 同一行・右寄せ・--text-muted
+-------------------------------------------+
```
...
**i18n（`src/i18n.ts` 拡張）**: 経過日数バッジ本文 `stagnantBadge(days)`（英 `${days}d`／日 `${days}日`）と、その `aria-label` `stagnantLabel(days)`（英 `Stale: not updated for ${days} days`／日 `滞留: ${days}日更新なし`）を両言語に足す（`i18n.test.ts` で両言語の欠けなしを固定）。設定タブのしきい値行文言（名前・説明。mtime 近似の明示を含む）も `SettingsMessages` に追加。

**設定タブ行の追加（`▸ 表示` 区分）**: しきい値（日数・0 でオフ）を数値入力で置く。

```
▸ 表示
  欠損ノートを未分類に表示   [ ●── ON ]
  滞留とみなす日数（0=オフ） [ 14 ]      ← #106。説明に「自動処理でも更新される」旨を明示
```
...
- **ロールバック（最新世代のみ）**: `onMoveCard` の Promise が reject したら、**その書き込みが当該 entry の最新世代のときだけ**保留移動を破棄して再描画（＝サーバ値の元象限へ戻る）。古い書き込みの失敗で後続の新しい移動の楽観状態を巻き戻さない。ロールバック判定（`rollbackFailedMove`／`isLatestGeneration`）と、settle の副作用計画（ロールバック要否・成功後の再突合・SR 通知種別・undo トースト表示要否）は純関数 `planSettle(failed, isLatest, hasUndo)`（`optimisticMove.ts`。`settleAnnouncement` を内包し `success`/`successUndoable`/`failure`/`silent` を返す）へ抽出し単体テストで固定する（`MatrixView.settle` は計画を機械的に実行する細い殻に留める＝dnd-kit のドロップ確定が jsdom で駆動できないため結線を純関数側で守る・レビュー指摘 #2）。`Notice` はアダプタが出す（AC3）。
- **成功確定時の再突合（レビュー指摘 #4）**: 書き込み成功の settle で in-flight が解消したら `reconcileNow()`（最新 `viewModel` を参照して `reconcilePendingMoves` を適用する内部ヘルパ）を呼んで保留を再突合する。in-flight 中にサーバ値（`onDataUpdated`）が `onMoveCard` 解決より先着していた場合、reconcile は in-flight 中は保留を残すため確定できず、settle が in-flight を 0 にするだけでは楽観オーバーレイが居残り後続のサーバ変更を隠す。再突合は `reconcilePendingMoves` の意味論（`server==pending` のときだけ落とす）そのままなので、サーバ未追従なら保留は残り**フリッカーは起きない**。
- **アンマウント後発火のガード（レビュー指摘 #5）**: ドラッグ中にビュー（リーフ）を閉じると `onunload`→`unmount` が走った後に書き込みが解決して `settle` が発火し、掃除対象外の 8 秒トーストタイマー予約＋アンマウント後 `setState` を起こしうる。`mountedRef`（マウント/アンマウントを追う ref）を settle 冒頭で見て、未マウントなら即 return する。
...
- **完了（`done:true`）成功後のフォーカス移譲**: Base の `done != true` フィルタ構成では当該カードが再クエリで unmount され、操作していたボタン/カードが消えてフォーカスが `body` へ落ちる。undo トーストと同じく `matrixSectionRef.current?.focus()`（`tabIndex=-1` のマトリクス領域）へ戻し、SR/キーボード利用者の読み進め位置の喪失を防ぐ（WCAG 2.4.3）。**ただし `dimCompleted`（＝`done != true` フィルタを張らない no-filter モードの目印）のときはカードが残る**ため、フォーカスは動かさず操作したボタン（いまは「未完了に戻す」ラベル）に留める（`if (done && !dimCompleted) focus()`＝残るのに section へ移すと読み進め位置を不要に失う・v0.2 レビュー確定）。未完了化（`done:false`）もカードが残るため動かさない。
...
- **ロックカード（`entry.locked`）のキーボード操作**: ロックカードは掴めない（`useDraggable({disabled})`）ため Space の掴み予約が無い。よって `role="button"` の標準操作に揃え、**Enter に加え Space でも「開く」を発火し、`preventDefault` で既定のペインスクロールを抑止する**（Space が無反応かつスクロールする壊れた挙動の是正・収束レビュー指摘。通常カードは Space=掴むのままで変えない）。
...
- **undo は完全復元（absent は delete で戻す）（人間承認済み）**: undo は「移動前の状態をそのまま復元する」意味論を採り、元が未分類（軸 absent）だった移動は frontmatter キーを **delete して未分類へ戻す**。`delete` はこの undo 経路のみで、分類ドラッグ（#20）は引き続き両軸明示・delete しない（v1 boolean 軸限定の制約を崩さない）。却下「厳密 no-delete（absent 由来は undo 不可）」: 制約は厳守できるが「元に戻す」が実際には戻らない驚きを生む。捕捉値は boolean に限定せず verbatim（生値）で保持し、非 boolean の元値もデータ破壊を残さず復元する（`bases.md` の `UndoRecord`）。
  - **undo 起動時に楽観オーバーレイ（`pending`）も落とす（トースト・コマンドの両経路で対称）**: undo は frontmatter を移動前へ戻すが、楽観移動の `pending` が残ると `reconcilePendingMoves` が「サーバ値≠保留・非 in-flight」で保留を落とせず、`applyPendingMoves` がカードを移動先象限へ描き続ける（ファイルは正しいのに表示が食い違う）。トースト undo は `dropPending(pending, entryId)`（`optimisticMove.ts`・純関数）で該当保留を明示的に落としてサーバ値の表示へ戻す。**`reconcilePendingMoves` の意味論は変えない**（`!confirmed && !in-flight` を残す既存挙動＝settle→confirm 間に無関係な再描画が来ても flicker させない、を保つ）。**コマンド経由 undo（`main.ts`）はこのコンポーネントを経由しないため、`MatrixView` がマウント時に「pending を落とす関数」を `MatrixCallbacks.registerPendingDropper` でアダプタへ登録し（アンマウントで `null` 解除）、`main.ts` が `runUndo` の戻り値（実際に復元した entryId）で生存ビューの `EisenhowerBasesView.dropPendingOverlay(entryId)` を呼んで各ビューの `pending` を落とす（トースト経路と挙動を対称化・レビュー指摘 #6）。** `registerPendingDropper` は plain function 型で UI は `obsidian` 型に触れない（AC5 の疎結合を維持）。
