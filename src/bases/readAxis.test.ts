...
describe("isUnsupportedAxisValue / hasUnsupportedAxisValue（非 boolean 上書き破壊の検出・ドラッグ不可ガード）", () => {
  ...
  it("hasUnsupportedAxisValue — getValue が throw しても false（ロックしない・throw を伝播しない・レビュー指摘 #3）", () => {
    // given: 書込可能 note.* 軸の getValue が throw（churn を模す）
    const entry = {
      file: { path: "Tasks/a.md", basename: "a" },
      getValue: (): Value => {
    ...
    // when / then: 例外を飲み込み false（破壊経路が無いため安全側でロックしない）＝ビュー全体を壊さない
    expect(() => hasUnsupportedAxisValue(entry, ids)).not.toThrow();
    expect(hasUnsupportedAxisValue(entry, ids)).toBe(false);
  });
