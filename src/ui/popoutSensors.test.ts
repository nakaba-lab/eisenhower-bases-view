...
describe("retargetListeners — リスナーを新 document へ張り替える中核", () => {
  ...
  it("空リスナー配列でも target が違えば popout へ張り替える（KeyboardSensor の遅延登録経路・レビュー指摘 #7）", () => {
    // given: keydown を setTimeout で遅延登録する KeyboardSensor は、retarget 実行時点で
    // listeners.listeners が空のことがある。だが target がメイン document のままだと、後で発火する
    // 遅延 add が誤った document へ張り、popout でキーボードドラッグを開始できない。
    const popoutDoc = document.implementation.createHTMLDocument("popout");
    const listeners = fakeListeners(document, []);
    // when
    retargetListeners(listeners, popoutDoc);
    // then: 空配列でも早期 return せず、target を popout へ張り替える（＝以後の遅延 add が popout に載る）
    expect(listeners.removedCount).toBe(1);
    expect(listeners.target).toBe(popoutDoc);
    expect(listeners.listeners).toEqual([]);
  });
