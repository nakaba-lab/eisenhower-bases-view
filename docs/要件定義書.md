...
| F7 | 滞留インジケータ | `file.stat.mtime` を読み、最終更新から N 日（既定 14・0 でオフ・ビュー options 主＋設定既定のハイブリッド）を超えたカードに控えめな滞留バッジ（時計＋経過日数・`--text-muted`）を付ける。読み取り専用ヒューリスティック（**書き込みゼロ・ネットワークゼロ**） |
