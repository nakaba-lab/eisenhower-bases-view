...
describe("mergeSettings — カード上の完了トグルの設定（#105 F10）", () => {
  ...
  it("mergeSettings — 書き戻しに使うプロパティ名（軸・完了）を前後トリムする（手編集の空白付きゴミキー書込を防ぐ・レビュー指摘）", () => {
    // given: data.json 手編集で前後空白付きのプロパティ名（completionProperty は #105 の書き戻しキーになる）
    const merged = mergeSettings({
      completionProperty: "done ",
      defaultUrgencyProperty: " urgent",
      defaultImportanceProperty: "  important  ",
    });
    // then: トリム済み（propertyId 完全一致で解決され、frontmatter["done "] のようなゴミキー書込を防ぐ）
    expect(merged.completionProperty).toBe("done");
    expect(merged.defaultUrgencyProperty).toBe("urgent");
    expect(merged.defaultImportanceProperty).toBe("important");
  });

  it("mergeSettings — cardBadgeProperties は空文字/空白のみ要素を落とす（1 枠を消費して実プロパティを押し出さない・レビュー指摘）", () => {
    // given: 手編集の data.json に空文字・空白のみが混ざる
    const merged = mergeSettings({
      cardBadgeProperties: ["note.due", "", "  ", "note.tags", 42, null],
    });
    // then: 非文字列も空文字/空白も除去し、実プロパティだけ残す（resolveBadgePropertyIds の不変条件を永続層で守る）
    expect(merged.cardBadgeProperties).toEqual(["note.due", "note.tags"]);
  });
});

describe("数値しきい値軸の設定（#121 v0.3-1a）", () => {
  it("DEFAULT_SETTINGS — 数値しきい値の既定は空文字（未設定＝数値軸オフ・v1 挙動を維持）", () => {
    // given/when/then: 既定は空＝数値軸を有効化しない（不意の配置を防ぐ off-sentinel）
    expect(DEFAULT_SETTINGS.defaultUrgencyThreshold).toBe("");
    expect(DEFAULT_SETTINGS.defaultImportanceThreshold).toBe("");
  });

  it("mergeSettings — 保存済みのしきい値文字列を per-axis で復元する（前後空白はトリム）", () => {
    // given: data.json に保存されたしきい値（設定タブのライブ編集と同じトリム正規化を復元経路にも適用）
    const merged = mergeSettings({ defaultUrgencyThreshold: " 3 ", defaultImportanceThreshold: "5" });
    // then
    expect(merged.defaultUrgencyThreshold).toBe("3");
    expect(merged.defaultImportanceThreshold).toBe("5");
  });

  it("mergeSettings — 数値で保存されていても文字列へ整える（手編集 data.json の防御）", () => {
    // given: 手編集で number が入っていても文字列表現へ寄せる（設定の型を string に統一）
    const merged = mergeSettings({ defaultUrgencyThreshold: 3, defaultImportanceThreshold: -2 });
    // then
    expect(merged.defaultUrgencyThreshold).toBe("3");
    expect(merged.defaultImportanceThreshold).toBe("-2");
  });

  it("mergeSettings — 欠損・非文字列/非数値は既定（空文字＝オフ）へ", () => {
    // given/when/then: 欠損やオブジェクト等の不正値は既定へ倒す
    expect(mergeSettings({}).defaultUrgencyThreshold).toBe("");
