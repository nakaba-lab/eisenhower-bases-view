...
- **v1 は boolean 軸限定**（v0.3 で数値/選択/タグ軸へ一般化＝下記）。ドラッグで `app.fileManager.processFrontMatter` により両軸を明示 `true/false` で書き戻し（楽観的更新＋`onDataUpdated` 整合＋失敗時ロールバック）
- **軸タイプの一般化（v0.3〜・#88/#91）**: boolean に加え **数値しきい値**（`value >= threshold`）・**選択**（文字列の trueValue/falseValue 完全一致）・**タグ**（frontmatter リストへの tagName 包含）軸を、読み取り（分類）と**越境時のみの書き戻し**（同じ側は値を温存・タグは他要素温存）で対応（`AxisSpec`）。象限ロジック（`classifyQuadrant`）は不変で boolean 軸は挙動不変。S0 実機スパイクで Value 表現・options 型を確定済み（§9）
...
- ~~数値しきい値・テキスト/タグ軸の書き戻し＝v2 以降~~ → **v0.3 でスコープ内に移行**（上記・#88/#91。旧「v2 以降」を解除）
- **インラインタグ（本文 `#tag`）軸・formula 由来の数値軸＝対象外**（`note.*` frontmatter リスト/数値/文字列のみ。formula/インラインは書き戻し不可制約のため恒久的に対象外）
...
| F11 | 軸タイプの一般化（v0.3・#88/#91） | F2/F3/F4 の軸を **boolean 固定から `AxisSpec`（boolean/数値/選択/タグ）へ一般化**。軸ごとに kind とパラメータ（数値=しきい値・代表値／選択=trueValue/falseValue／タグ=tagName）をビュー options（`dropdown`+`slider`/`text`・条件付きは `shouldHide`）＋設定既定のハイブリッドで指定。読み取りは `interpretAxis(raw, spec)`（`instanceof` で Value 判別・公開 API で取り出し）、書き戻しは `planAxisWrite`（**同じ側は書かず値温存・越境時のみ代表値／タグは配列 add/remove で他要素温存**）。`classifyQuadrant` は不変で **boolean 軸は挙動不変**。同一キーガードは kind-aware（tag×tag×異 tagName を合法化）。undo は #105 のキーリスト機構を流用（タグは値等価で補強）。詳細設計は `docs/superpowers/specs/2026-07-11-axis-generalization-v0.3-design.md`・S0 実機スパイクで前提確定（§9） |
...
### S0 スパイク（v0.3 軸の一般化・#88/#91）で確定した事項（2026-07-12）

実機 Obsidian **1.12.7** で `scripts/e2e/probe-s0.{sh,js}`（CDP プローブ）を実行し、数値/文字列/タグ/absent の
`getValue` の Value 表現を introspect ＋ `obsidian.d.ts` で公開 API・export 名を確定した。詳細設計は
`docs/superpowers/specs/2026-07-11-axis-generalization-v0.3-design.md` §4。**軸の一般化（数値/選択/タグ）着手の技術前提が揃った。**

| 項目 | 確定内容 |
|------|---------|
| Value 表現・判別 | 数値=**`NumberValue`**・文字列=**`StringValue`**・タグ=**`ListValue`**（要素 **`TagValue`**・`#` 前置）・欠損=`NullValue`・boolean=`BooleanValue`。全て obsidian が export（`obsidian.d.ts` で確認）。実機は全 Value が minified constructor `"t"` のため **`instanceof` で判別**（名前・`toString()` 非依存＝#33 の教訓） |
| primitive の取り出し | `PrimitiveValue<T>` に公開 getter が無いため **公開 API**で取る: 数値=`Number(v.toString())`・文字列=`v.toString()`。内部 `.data` には頼らない（churn 耐性）。リストは `.data` 手パースでなく **`ListValue.includes(new TagValue(name))`／`length()`／`get(i)`** を使う |
| タグの `#` 前置 | frontmatter `tags: [urgent]` は Value 層で **`#urgent`**（TagValue・`lowerTag` 併持）。包含は loose-equals の `includes` に委ね、`#`/大小差の吸収は 3b の結合フェーズで実機確認 |
| `getValue` のエラー | エラーは **`ErrorValue`** で返る（formula 失敗等・`obsidian.d.ts` 明記）→ `readAxisValueSafely` の try/catch に加え `instanceof ErrorValue` も unsupported（安全側ロック）へ倒す |
| Configure view options 型 | `registration.options` は **`(config) => BasesAllOptions[]`（config 関数）**。使える型は `dropdown`/`slider`/`text`/`multitext`/`toggle`/`property`/`file`/`folder`/`formula`、各 option に **`shouldHide?()`**、折りたたみ **`group`** あり。→ **kind 選択（dropdown）＋条件付きフィールド（options 分岐＋shouldHide）がネイティブに実現可能**（縮退案不要） |
| obsidianStub 追補 | 単体テスト用に `ListValue`/`TagValue`/`ErrorValue`（＋既存 `NumberValue`/`StringValue`）を実機表現に合わせて `src/test-support/obsidianStub.ts` へ追加する |
...
| タグ軸の absent/false 意味論・select の 3 値運用（#88/#91） | tag は「キー無し=未分類/タグ無し=false」で暫定・select は二値必須（medium は locked） | 各 L2（select=2・tag=3b）の実装前設計で設計オプション比較＋人間承認（`docs/superpowers/specs/…design.md` §3.6） |
...
| 2026-07-12 | **v0.3「軸の一般化」を決定**（#88 数値/#91 選択・タグ）。§2 の「数値/テキスト/タグ軸の書き戻し＝v2 以降」の**将来送りを解除**しスコープ内へ、§3 に **F11 軸タイプの一般化** を追加。**S0 実機スパイク（Obsidian 1.12.7・`scripts/e2e/probe-s0.*`）で Value 表現（Number/String/List+Tag/Error の instanceof 判別・公開 API 取り出し）と Configure view options 型（config 関数＋dropdown/slider/text/group/shouldHide で kind 選択＋条件付きが可能）を確定**（§9 に S0 節を追加）。象限ロジックは不変で boolean 軸は挙動不変。基本設計は `docs/superpowers/specs/2026-07-11-axis-generalization-v0.3-design.md`。以降は `/github-planning` で L2 起票→ `/dev-tasks` で実装 | Takahiro.N |
