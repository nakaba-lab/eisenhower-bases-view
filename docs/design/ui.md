...
> 起点は `docs/要件定義書.md`「UI/UX 方針」節。F1（#18）のシェル＋状態表示に続き、#19（F2）で 2×2 グリッド＋未分類ゾーンの配置を実装し `status: active` に確定した。**#20（F3）のドラッグ書き戻しを積み増すため一時的に `status: draft` に戻している（実装完了後の「ドキュメント更新」で `active` に確定する）。**
>
