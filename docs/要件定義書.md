...
| F7 | カード追加プロパティ表示 | カードにタイトル下へ**読み取り専用**バッジで追加プロパティ（期日・タグ・プロジェクト等）を最大 3 個表示。ビュー options（主）＋プラグイン設定（デフォルト）で選択。読み取り専用のため `formula.*`／`file.*` も可。期日らしい値（厳格 ISO・今日以前）は任意トグルでアクセント強調。既定は表示 0 個（現状維持） |
