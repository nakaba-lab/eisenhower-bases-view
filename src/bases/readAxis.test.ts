...
describe("axesShareWritableKey — kind-aware 化（タグ軸基盤・#124）", () => {
  // 判定に使う AxisSpec ビルダ（src/logic/axis の正準型を流用）。
  const tag = (name: string): AxisSpec => ({ kind: "tag", tag: name });
  ...
  // 両軸が同一の書き戻し可能キー note.tags を指す（設定として同一キー）。
  const sameKey: AxisPropertyIds = {
    urgent: "note.tags" as BasesPropertyId,
    important: "note.tags" as BasesPropertyId,
  };

  it("AC1: tag×tag・同一キー・異 tagName は合法（false＝ロックしない）", () => {
    // given: tags 1 本に urgent/important の 2 タグ（同一キー・異 tagName＝タグ軸の本命）
    const specs = { urgent: tag("urgent"), important: tag("important") };
    // when / then: タグ配列は別タグが安全に同居できるため衝突ではない（AC1）
    expect(axesShareWritableKey(sameKey, specs)).toBe(false);
  });

  it("AC2: tag×tag・同一キー・同一 tagName は設定ミス（true）", () => {
    // given: 同じタグを 2 軸が toggle し合う（互いに潰す）
    const specs = { urgent: tag("urgent"), important: tag("urgent") };
    expect(axesShareWritableKey(sameKey, specs)).toBe(true);
  });

  it("AC3 回帰: boolean×boolean・同一キーは従来どおり設定ミス（true）", () => {
    const specs = { urgent: BOOL, important: BOOL };
    expect(axesShareWritableKey(sameKey, specs)).toBe(true);
  });

  it("AC3 回帰: number×number・同一キーは設定ミス（true）", () => {
    const specs = { urgent: NUM, important: NUM };
    expect(axesShareWritableKey(sameKey, specs)).toBe(true);
  });

  it("AC3 回帰: select×select・同一キーは設定ミス（true）", () => {
    const specs = { urgent: SEL, important: SEL };
    expect(axesShareWritableKey(sameKey, specs)).toBe(true);
  });

  it("異 kind・同一キー（tag×boolean）は共存不能なので設定ミス（true・対称）", () => {
    // frontmatter の 1 キーは 1 値型しか持てず共存不能＝衝突。順序に依らず対称。
    expect(axesShareWritableKey(sameKey, { urgent: tag("urgent"), important: BOOL })).toBe(true);
    expect(axesShareWritableKey(sameKey, { urgent: BOOL, important: tag("urgent") })).toBe(true);
  });

  it("tagName は完全一致・大文字小文字を区別（Urgent ≠ urgent は合法）", () => {
    // interpretAxis の includes（大小区別）と同一意味論に揃える
    const specs = { urgent: tag("Urgent"), important: tag("urgent") };
    expect(axesShareWritableKey(sameKey, specs)).toBe(false);
  });

  it("異キー・tag 指定でも false（衝突対象外＝別キーは共存できる）", () => {
    const ids: AxisPropertyIds = {
      urgent: "note.tags" as BasesPropertyId,
      important: "note.labels" as BasesPropertyId,
    };
    const specs = { urgent: tag("urgent"), important: tag("important") };
    expect(axesShareWritableKey(ids, specs)).toBe(false);
  });

  it("非 note.*（formula）同一キーは tag 指定でも false（書込不可は別ガードが弾く）", () => {
    const ids: AxisPropertyIds = {
      urgent: "formula.tags" as BasesPropertyId,
      important: "formula.tags" as BasesPropertyId,
    };
    const specs = { urgent: tag("urgent"), important: tag("important") };
    expect(axesShareWritableKey(ids, specs)).toBe(false);
  });

  it("specs 省略時は両軸 boolean 既定＝現行呼び出しは挙動不変（同一キー→true）", () => {
    expect(axesShareWritableKey(sameKey)).toBe(true);
  });
});
...
describe("readAxisReadings — 数値しきい値軸の kind-aware 読み取り（#121 v0.3-1a）", () => {
  ...
  it("AC1（1b 解禁）— 有限数の数値軸カードは配置され unlock（掴める＝書き戻し解禁・#122）", () => {
    // given: 緊急軸に数値 9（threshold=5）
    const entry = mockEntry({ "note.urgent": new NumberValue(9), "note.important": TRUE });
    // when
    const readings = readAxisReadings(entry, ids, thresholds);
    // then: 正しい象限側に置かれ、かつ unlock（1b で掴める＝ドラッグ書き戻しを解禁する）
    expect(readings.urgent.side).toBe(true);
    expect(readings.urgent.locked).toBe(false);
  });
...
describe("readAxisValues / hasUnsupportedAxisValue — threshold 引数で数値軸を認識（#121）", () => {
  ...
  it("hasUnsupportedAxisValue — threshold 設定の有限数値軸カードは unlock（1b 解禁・掴める・#122）", () => {
    const entry = mockEntry({ "note.urgent": new NumberValue(9), "note.important": TRUE });
    expect(hasUnsupportedAxisValue(entry, ids, { urgent: 5, important: null })).toBe(false);
  });
