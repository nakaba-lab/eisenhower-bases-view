...
| 2026-07-14 | **F10 完了トグルの既定を反転**（主要機能表 F10 行を改訂）: 完了プロパティの既定を空（opt-in・機能オフ）から `done`（初期状態で有効）へ変更し、設定なしでもカードに完了ボタンが出るようにした（軸プロパティが既定で `urgent`/`important` を持つのと同型・利用者報告「設定するまでボタンが出ない」への対応）。欄を空にすれば opt-out。**移行**: 新規インストール・完了プロパティ未保存・旧 v0.1.x は既定で有効、v0.2 系（0.2.0/0.2.1）で設定を保存済みの環境は当時の既定だった空文字が `data.json` に永続化されているため無効のまま維持（`mergePropertyName` が明示空を尊重・既定変更で勝手に有効化しない）。多角的レビューで確定。設計は `docs/design/ui.md`・`bases.md`（status: active） | Takahiro.N |
