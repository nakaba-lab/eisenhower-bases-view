...
## 軸タイプの一般化（AxisSpec・v0.3 #120 F0〜・`status: draft`→段階的に active）

v0.3 で軸を boolean 固定から `AxisSpec`（boolean/number/select/tag）へ一般化する（要件 F11・#88/#91）。churn 面を最小化するため、**象限ロジック（`classifyQuadrant`／`axisValuesForQuadrant`）は一切変えず**、軸解釈の 2 つの端だけを一般化する:

```
[Value 生値] --interpretAxis(raw, spec)--> [side: boolean|undefined] --classifyQuadrant--> [象限]
[象限] --axisValuesForQuadrant--> [side: boolean] --planAxisWrite(side, spec, current)--> [書き込み or null]
```

`boolean | undefined`（軸の側・未分類）が軸解釈と象限判定の**安定インターフェース**で、この不変性が「boolean 軸は挙動不変」を保証する。

### 責務分担（純ロジック / アダプタ）

- **純ロジック（`src/logic/axisSpec.ts`・#120 F0・実装済み）**: `AxisSpec`／`AxisRaw`（型付き生値）／`interpretAxis(raw, spec) → { side, locked }`／`planAxisWrite(side, spec, current) → { value } | null` を Obsidian 非依存で持つ（単体 TDD・47 ケース）。**Value 型に非依存**＝アダプタが正規化した `AxisRaw` だけを見るため S0 スパイク非依存で先行実装できた。
  - `interpretAxis`: `absent`→未分類・ドラッグ可、`unsupported`→未分類・安全側ロック。present 値は kind に照らして解釈し、解釈不能な present 値は `locked`（#34 の破壊防止を kind-aware に一般化）。number は `value>=threshold`（非有限は locked）、select は 2 値完全一致（未知値は locked）、tag はリスト包含（`normalizeTag` で `#` 前置・大小差を吸収）。
  - `planAxisWrite`: **「同じ側なら書かない（値温存）・越境時のみ書く」**。boolean は常に書く（挙動不変）、number は代表値（`highValue??threshold`／`lowValue??threshold-1`）、select は `trueValue`/`falseValue`、tag は配列 add/remove（**他要素温存・全置換しない**）。書いた軸だけを undo（#105 の keylist 機構）へ渡す。
- **アダプタ（`src/bases/readAxis.ts`・#121 1a / #123 2 / #125 3b で配線）**: `entry.getValue` の Value を `AxisRaw` へ正規化する（**instanceof で判別・公開 API で取り出す**）。S0 実機スパイクで確定（要件 §9）: 数値=`instanceof NumberValue`→`Number(v.toString())`、文字列=`instanceof StringValue`→`v.toString()`、タグ=`instanceof ListValue`→`includes(new TagValue)`/`get`/`length`（要素は `#` 前置の `TagValue`）、`getValue` エラーは `ErrorValue`→`unsupported`。`.data`（内部）に頼らない。`obsidianStub.ts` に `NumberValue`/`StringValue`/`ListValue`/`TagValue`/`ErrorValue` を実機表現に合わせて追加する。

### 同一キーガードの kind-aware 化（#124 3a）

`axesShareWritableKey`（2 軸の frontmatter キー直接比較）は、タグ軸の本命「`tags` 1 本に urgent/important の 2 タグ」（同一キー・異 tagName＝合法）をブロックしてしまうため、kind を考慮した判定へ改修する（boolean/number/select の同一キーは従来どおり設定ミスで弾く）。

### 段階（L2）

F0（純ロジック・済）→ 1a（数値 読み取り/表示・locked 段階で出荷可）→ 1b（数値 書き戻し+undo）→ 2（select）→ 3a（同一キーガード）→ 3b（tag）。基本設計は `docs/superpowers/specs/2026-07-11-axis-generalization-v0.3-design.md`。
