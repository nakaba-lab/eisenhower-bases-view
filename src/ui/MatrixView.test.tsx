...
describe("MatrixView — DragOverlay の body への portal（#43 回帰ガード）", () => {
  ...
  async function grabFirstCard(container: HTMLElement): Promise<HTMLElement> {
    ...
    await waitFor(() => {
      if (!doc.body.querySelector(".eisenhower-note-card--overlay")) throw new Error("overlay 未描画");
    });
    return card;
  }
  ...
  it("ドラッグ中（Space で掴む）にオーバーレイが render コンテナの外＝body 側へ portal される", async () => {
    ...
    // ドラッグを cancel してセンサーのリスナーを回収（afterEach も担うが明示する）
  ...
  it("portal 先はグローバル document.body ではなくビューの ownerDocument.body（別 document で確認）", async () => {
    // 2dfbb2d の実変更（document.body → matrixSectionRef.current?.ownerDocument.body）を固定する。
    // 単一 document の jsdom では ownerDocument.body===document.body で区別できないため、別 document を
    // 用意しそこへ container を置く。overlay がグローバル document.body ではなく、その別 document.body に
    // 出ることを確認する（document.body グローバル固定へ戻す回帰＝popout でメイン窓に出て消えるバグを捕らえる）。
