...
describe("firstSharedWritableKey（N キー衝突検出＝#105 で axesShareWritableKey を一般化）", () => {
  it("firstSharedWritableKey — 書込可能キーの最初の重複を返す（3 キー総当たり）", () => {
    // given: 緊急/重要は別、完了が緊急と同一キー（note.urgent）
    const ids = [
      "note.urgent" as BasesPropertyId,
      "note.important" as BasesPropertyId,
      "note.urgent" as BasesPropertyId,
    ];
    // when / then: 重複キー "urgent" を返す
    expect(firstSharedWritableKey(ids)).toBe("urgent");
  });

  it("firstSharedWritableKey — 全キーが別なら null（正常設定）", () => {
    const ids = [
      "note.urgent" as BasesPropertyId,
      "note.important" as BasesPropertyId,
      "note.done" as BasesPropertyId,
    ];
    expect(firstSharedWritableKey(ids)).toBeNull();
  });

  it("firstSharedWritableKey — 非 note.*（formula/file）や null は無視する（書込不可は衝突対象外）", () => {
    // given: formula.* が 2 つ・file.* が 1 つ・null が 1 つ。いずれも書込不可なので衝突と見なさない
    const ids = [
      "formula.score" as BasesPropertyId,
      "formula.score" as BasesPropertyId,
      "file.name" as BasesPropertyId,
      null,
    ];
    expect(firstSharedWritableKey(ids)).toBeNull();
  });

  it("firstSharedWritableKey — axesShareWritableKey は本関数の 2 キー適用と一致する（一般化の整合）", () => {
    const shared: AxisPropertyIds = {
      urgent: "note.same" as BasesPropertyId,
      important: "note.same" as BasesPropertyId,
    };
    const distinct: AxisPropertyIds = {
      urgent: "note.urgent" as BasesPropertyId,
      important: "note.important" as BasesPropertyId,
    };
    expect(axesShareWritableKey(shared)).toBe(
      firstSharedWritableKey([shared.urgent, shared.important]) !== null,
    );
    expect(axesShareWritableKey(distinct)).toBe(
      firstSharedWritableKey([distinct.urgent, distinct.important]) !== null,
    );
  });
});

describe("resolveCompletionId / resolveCompletionKey（#105 F10 完了プロパティ解決・opt-in・3 キーガード）", () => {
  it("resolveCompletionId — 既定（settings 空・config 未設定）は null＝機能オフ（opt-in）", () => {
    // given / when / then: completionProperty 既定は "" のため完了トグルは無効
    expect(resolveCompletionId(null, DEFAULT_SETTINGS)).toBeNull();
    expect(resolveCompletionKey(null, DEFAULT_SETTINGS)).toBeNull();
  });
