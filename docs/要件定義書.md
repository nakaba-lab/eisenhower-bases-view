...
| **absent / false / true の区別（実装必須）** | `true`→`toString()="true"`/`isTruthy()=true`、`false`→`toString()="false"`/`isTruthy()=false`、**`absent`→`toString()=null`/`isTruthy()=false`**。**absent と false は `isTruthy()` では区別不可。`getValue(...)?.toString() === null`（NullValue）で absent を判定する**こと（さもないと欠損ノートが「最低象限」に誤分類される＝スパイクで実証） |
