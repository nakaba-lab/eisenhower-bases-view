...
describe("isUndoApplicable — 配列（タグ軸）は値等価で照合する（#125 AC4）", () => {
  // タグ軸の書き戻しは frontmatter の tags 配列を add/remove した**新配列**を書く。
  // processFrontMatter の round-trip 後、frontmatter[key] は wrote とは別オブジェクトになるため、
  // `===`（参照比較）では常に不一致になり undo が一度も適用されない。要素単位の値等価で照合する。
  const tagRecord = (wrote: unknown): UndoRecord => ({
    entryId: "t.md",
    title: "t",
    entries: [{ key: "tags", previous: { present: true, value: ["work"] }, wrote }],
  });

  it("isUndoApplicable — 別オブジェクトでも要素が同値の配列なら true（参照非同一でも適用可）", () => {
    // given: wrote と frontmatter の tags は別オブジェクトだが要素は同値
    const wrote = ["work", "urgent"];
    const frontmatter: FrontmatterLike = { tags: ["work", "urgent"] };
    // when / then: 値等価で一致 → 適用可
    expect(isUndoApplicable(frontmatter, tagRecord(wrote))).toBe(true);
  });

  it("isUndoApplicable — 要素が異なる配列なら false（外部で書き換えられた＝誤適用しない）", () => {
    // given: 書き込み後にユーザーがタグを足した
    const frontmatter: FrontmatterLike = { tags: ["work", "urgent", "later"] };
    // when / then: 長さ差 → 不一致
    expect(isUndoApplicable(frontmatter, tagRecord(["work", "urgent"]))).toBe(false);
  });

  it("isUndoApplicable — 順序が異なる配列は false（要素順も含めて同値照合）", () => {
    // given: 同じ要素だが順序違い（add/remove は末尾追加のため順序は保存される前提）
    const frontmatter: FrontmatterLike = { tags: ["urgent", "work"] };
    // when / then: 順序差 → 不一致（自分が書いた配列そのままではない）
    expect(isUndoApplicable(frontmatter, tagRecord(["work", "urgent"]))).toBe(false);
  });

  it("isUndoApplicable — 空配列どうしは値等価で true（全タグ除去の書き戻し）", () => {
    // given: false 側ドロップで全 tag を除いた空配列を書いた
    const frontmatter: FrontmatterLike = { tags: [] };
    // when / then
    expect(isUndoApplicable(frontmatter, tagRecord([]))).toBe(true);
  });

  it("isUndoApplicable — frontmatter 側が非配列なら false（型が変わった＝誤適用しない）", () => {
    // given: tags が配列でなくなっている
    const frontmatter: FrontmatterLike = { tags: "urgent" };
    // when / then
    expect(isUndoApplicable(frontmatter, tagRecord(["work", "urgent"]))).toBe(false);
  });

  it("isUndoApplicable — 非配列の wrote は従来どおり === で照合（回帰・boolean）", () => {
    // given: boolean 書き込みは参照比較のまま（プリミティブは === で十分）
    const record: UndoRecord = {
      entryId: "b.md",
      title: "b",
      entries: [{ key: "urgent", previous: { present: false }, wrote: true }],
    };
    // when / then
    expect(isUndoApplicable({ urgent: true }, record)).toBe(true);
    expect(isUndoApplicable({ urgent: false }, record)).toBe(false);
  });
});
