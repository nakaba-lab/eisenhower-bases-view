...
- **v1 は boolean 軸限定＝正の許可リスト `instanceof BooleanValue`（#34）**: `note.*` 接頭辞ガード（書き戻し可能性）に加え、**値の型が `BooleanValue` の軸だけ** `isTruthy()` で boolean 化し、それ以外（`NullValue`＝absent・`NumberValue`・`StringValue` 等）は `undefined`（未分類）へ退避する。非 boolean の `note.*`（例: 数値 `note.priority: 3`）を軸に向けても 4 象限に並ばず未分類へ落ちるため、**無操作での自動配置＝ドラッグ露出**（4 象限のカードがそのまま掴めてドロップで `true/false` 上書き→数値/文字列破壊）という最も起きやすい経路を断つ。
  - **未分類からの手動ドラッグ破壊を UI ロックで封鎖（リリース前の詰めで解消）**: #34 は**読み取り側**（当時の `normalizeAxis`＝#121 で `toAxisRaw`＋`readSingleAxisReading` に統合）を boolean に狭めて 4 象限への自動配置＝ドラッグ露出は断ったが、未分類ゾーンのカードは `useDraggable` のままで、手動で 4 象限へドロップすると書き戻し側ガード `resolveWritableAxisKeys` は `note.*` を書込可能と判定して通過し（boolean 型を検査しない）非 boolean 値が破壊されうる残存点があった（当初 F4/#21 送り）。リリース前レビューで data-loss として再提起され封鎖した: `hasUnsupportedAxisValue(entry, ids)`（書込可能 `note.*` 軸に非 boolean 値を持つか。`isUnsupportedAxisValue` が `NullValue`/null と `BooleanValue` を false にして「absent」と「非 boolean 在中」を区別）で検出し `MatrixEntry.locked` を立て、`NoteCard` が locked カードを `useDraggable({disabled:true})`＝**ドラッグ不可**にして淡色＋鍵アイコンでマークする（クリックで開く導線は残しユーザーが値を直せる）。真に absent なカード（欠損）は `locked` を付けず従来どおり分類ドラッグ可。これで読み取り側（未分類化）＋ UI 側（ドラッグ不可）＋書き戻し側（`resolveWritableAxisKeys` の note.* ガード）で非 boolean 破壊経路を塞いだ（UI 詳細は `ui.md`、検証は `readAxis.test.ts`／`NoteCard.test.tsx`）。
...
- **Bases 接触点（`getValue`／`getAsPropertyId`）の例外を境界で退避しビュー全体を守る（churn 耐性・レビュー指摘 #3・収束レビュー #2/#3）**: 軸読み取り `entry.getValue(id)` と軸解決 `config.getAsPropertyId(key)` は churn 対象の Bases API 接触点で、型契約は `Value | null`／`BasesPropertyId | null` だが未対応プロパティ型・内部状態不整合・API 破壊的変更で throw しうる。読み取り例外が `toViewModel`→`renderCurrent`→`onDataUpdated` まで伝播すると**マトリクス全体の再描画が壊れ**、正常カードも巻き添えになる。
  - **`getValue`**: `readAxisValueSafely(entry, id)` で try/catch し、成功は `{ ok:true, value }`・**例外は `{ ok:false }`** で返す（「値が `null`＝absent だった」と「読み取り自体が throw した＝型を確証できない」を区別）。呼び出し側は**用途ごとに安全側の既定**へ倒す: 配置（当時 `readSingleAxis`＝#121 で `readSingleAxisReading`）は throw→`undefined`（未分類）、**ロック判定（当時 `isUnsupportedOnWritableAxis`＝#121 で `readAxisReadings` の `locked` に統合）は throw→`true`（ロック）**。後者が重要で、読み取り（`getValue`）と書き戻し（`processFrontMatter`）は**別系統**（書き戻しは `getValue` を経由せず生 frontmatter を `true/false` 上書き）のため、throw を absent と同一視してロックし損ねると非 boolean 値を持つカードがドラッグ可能になり上書き破壊しうる（#34/`hasUnsupportedAxisValue` が塞いだ経路の再開＝収束レビュー #2 で是正）。同一 `propertyId` の失敗は `loggedGetValueFailures`（Set）で一度だけ `console.error` する（再描画毎・軸毎のログ洪水抑制＝収束レビュー #4）。
...
**完了プロパティの解決（軸と同型のハイブリッド）**: 設定 `completionProperty: string`（**既定 `"done"`＝初期状態で有効**。軸が `urgent`/`important` を既定に持つのと同型で、設定なしでも完了ボタンが出る。明示的な空文字で opt-out）＋ビュー option `COMPLETION_OPTION_KEY = "completionProperty"`。**設定の読込（`mergeSettings`）は書き戻しに使うプロパティ名（軸・完了）を `mergePropertyName` で前後トリム**する（設定タブの入力時トリムと一致させ、手編集 data.json の `"done "` 等が propertyId 完全一致で解決されず `frontmatter["done "]` のような空白付きゴミキーへ書き込まれるのを防ぐ・v0.2 レビュー）。`resolveCompletionKey(config, settings)` が `config.getAsPropertyId` 主・設定デフォルトで解決し、**書き戻し可能な `note.<key>` のみ**（`isWritableAxisProperty` を共有）frontmatter キーを返す。空・非 `note.*`・軸と衝突（下記 3 キーガード）なら `null`＝機能無効（チェックボタンを出さない）。Configure view セレクタは `buildCompletionViewOption(messages)`（`buildAxisViewOptions` と同型・`filter: isWritableAxisProperty`）。
...
## AxisSpec 基盤（軸の一般化・#120 F0・`status: active`）

> **#120 を実装し `status: active` に確定した（2026-07-15・人間承認済み）**。本節は v0.3「軸の一般化（数値/選択/タグ軸）」の**純ロジック基盤**の設計。実装は `src/logic/axis.ts`（Obsidian 非依存・単体 TDD）に閉じ、アダプタ配線（Obsidian `Value`→`AxisRaw` の `instanceof` 振り分け）は後続 L2（数値/選択/タグ各軸）のスコープ。
>
> ⚠️ **設計書の再構成（人間承認済み・2026-07-15）**: Issue #120 が真実源と指す `docs/superpowers/specs/2026-07-11-axis-generalization-v0.3-design.md`（§3・§3.3 表）は**リポジトリ・全 git 履歴に存在しない**。AC（Given/When/Then）から §3.3 相当を再構成し、下記 4 論点を `AskUserQuestion` で確定して本節を設計の真実源とした（実物の spec が後日出た場合は本節と差分照合する）。

### 責務（このユニットは何をするか）

軸を v1 の boolean 固定から `AxisSpec`（boolean/number/select/tag）へ一般化する**純関数基盤**。`interpretAxis`（生値→軸の側 `boolean|undefined`＋`locked`）と `planAxisWrite`（目標の側→書き込みプラン `|null`）を提供し、後続の各軸タイプ L2（アダプタ配線）が共有する。象限ロジック（`classifyQuadrant`／`axisValuesForQuadrant`）は**不変**に保ち、`boolean | undefined`（軸の側＝`AxisValues.urgent/important`）を安定インターフェースとして維持する（象限側は #120 で触らない）。undo は #105 のキーリスト機構（`UndoEntry`／`wrote: unknown`・`AxisWrite {key, value: unknown}`）をそのまま流用（拡張不要）。
...
/** 型正規化済みの生値（アダプタが Obsidian Value を instanceof で振り分けて渡す＝型タグ付き判別共用体）。 */
export type AxisRaw =
  | { kind: "absent" }
...
/** interpretAxis の結果。side=undefined は当該軸が未分類、locked=true はドラッグ不可（既存値保護）。 */
export interface AxisReading { side: boolean | undefined; locked: boolean }

/** planAxisWrite の結果（書き込む値のプラン。key はアダプタが解決して付す＝純層は key 非依存）。 */
export interface AxisWritePlan { value: unknown }
```
...
```mermaid
flowchart LR
  V["Obsidian Value<br/>(BooleanValue / NumberValue / …)"] -->|"アダプタ: instanceof 振り分け"| R["AxisRaw"]
  R --> IA["interpretAxis(raw, spec)"]
  SPEC["AxisSpec"] --> IA
  IA --> RD["AxisReading {side, locked}"]
  RD -->|"side を両軸ぶん集約"| CQ["classifyQuadrant(AxisValues) — 不変"]
  Q["ドロップ先 Quadrant"] --> AV["axisValuesForQuadrant — 不変<br/>→ 目標 side(boolean)"]
  AV --> PW["planAxisWrite(side, spec, current)"]
  SPEC --> PW
  R --> PW
  PW -->|"null=温存 / {value}"| WR["アダプタ: key を付け AxisWrite{key,value}<br/>→ buildUndoEntries + processFrontMatter"]
```

### interpretAxis(raw, spec) → AxisReading（§3.3 再構成表）

セル記法 `side / locked`。**未対応セル**（生値の型が spec と不一致）＝`undefined / true`（present だが解釈不能＝既存値をドラッグ上書きから保護）。**absent**＝全 kind で `undefined / false`（何も無いので保護不要・ドロップで代表値を書ける）。

| spec ＼ raw | absent | boolean | number | string | array |
|---|---|---|---|---|---|
| **boolean** | undefined / false | **value / false** | undefined / true | undefined / true | undefined / true |
| **number** | undefined / false | undefined / true | **value≥threshold ? true : false / false**（NaN・±Inf → **undefined / true**） | undefined / true | undefined / true |
| **select** | undefined / false | undefined / true | undefined / true | **==trueValue→true・==falseValue→false / false**／**どちらとも不一致→undefined / true（保護）** | undefined / true |
| **tag** | undefined / false | undefined / true | undefined / true | undefined / true | **tag∈array→true・else→false / false** |

- **number 境界**: `value >= threshold` で true 側（**境界ちょうどは true 側**・AC2）。NaN／±Inf は `side=undefined・locked=true`（AC3）。
- **select 異物値**（trueValue/falseValue のどちらとも一致しない文字列）: `undefined / true`＝保護（決定 #3）。
- **tag 不在**（配列は在るが対象タグを含まない）: `false` 側に確定（決定 #4。tag 所属は二値）。tag×array は常に `locked=false`（配列は add/remove で安全に書ける）。

### planAxisWrite(side, spec, current) → AxisWritePlan | null

`side`＝ドロップ先象限が要求する目標の側（`axisValuesForQuadrant` 由来の boolean）。`current`＝現在の `AxisRaw`。

| spec.kind | 既に目標側（→ null＝値温存・AC4） | 越境／absent（→ 代表値・AC5/AC6） |
|---|---|---|
| **boolean** | （温存しない）**常に `{value: side}`**＝true/false を無条件に書く（**挙動不変**・AC7） | 同左 |
| **number** | current が有限数で `interpretAxis` の side が目標と一致 → `null`（数値温存） | `{value: side ? (highValue ?? threshold) : (lowValue ?? threshold - 1)}` |
| **select** | current 文字列が目標側の値（trueValue/falseValue）と一致 → `null` | `{value: side ? trueValue : falseValue}` |
| **tag** | 目標所属と現配列が既に一致（add/remove 不要）→ `null` | `{value: <配列に tag を add（side=true）/ remove（side=false）した新配列。他要素は温存>}`（AC6） |

- **「既に目標側か」は `interpretAxis(current, spec).side === side` で一律判定**する（number/select/tag 共通）。boolean のみこの判定を経ず**常に** `{value: side}` を返す（AC7 の「常に true/false」）。
- **tag の absent は書く**（`[]`）: absent は `interpretAxis` で未分類（`undefined`）＝目標 false と不一致のため、`side=false` でも `null` にせず**空配列 `[]` を書いて false 側を確定**させる（`array []` は `interpretAxis` で false と読める）。`side=true` は `[tag]` を書く。number/select の absent も同様に代表値を書く（AC5）。
- **locked な current は保護（number/select/tag は `null`）**: present だが spec で解釈できない値（select 異物値・数値軸への文字列・NaN 等）は上書きせず `null` を返す（決定#3・#34 の一般化を planAxisWrite 側でも二重化）。通常 locked のカードは UI がドラッグを封じるが、基盤モジュール自身も破壊を拒む（防御的深層防御・レビュー should）。**boolean は AC7 のためこの保護の対象外＝常に `{value: side}`**（非 boolean 値の保護は #34 の読み取り側〔未分類化〕・UI ロック・書き戻し側 `note.*` ガードで担保）。
- **number の `threshold-1` フォールバックの注意（後続 number 軸 L2 へ引き継ぎ）**: `lowValue` 未設定時の false 側代表値 `threshold - 1` は、`threshold` が極端に大きい場合（例 `1e20`）に浮動小数の丸めで `threshold - 1 === threshold` となり `interpretAxis` で true 側に読み戻されうる。現実の優先度しきい値（小さい整数）では起こらないが、数値軸の L2 が任意しきい値を露出する段で `lowValue` 未設定時のフォールバックを見直す（レビュー nit）。

### 主要な設計判断（現行の理由）

- **key を純層に持ち込まない（`AxisWritePlan {value}` を返す・決定と直結）**: `AxisSpec` は Obsidian 非依存に保つため、書き戻し先の `BasesPropertyId`（`note.<key>`）は純層に含めない。`planAxisWrite` は書き込む**値だけ**を返し、アダプタが解決済み key を付けて undo の `AxisWrite {key, value}`（`src/logic/undo.ts`・#105）へ組む。これで #105 の undo 機構（`buildUndoEntries`／`applyUndo`／`isUndoApplicable`・verbatim `wrote: unknown`）を拡張なしで流用できる。
- **AxisRaw は型タグ付き判別共用体（決定 #2）**: アダプタが Obsidian `Value` を `instanceof`（`BooleanValue`/`NumberValue`/`StringValue`/`NullValue`/配列）で `AxisRaw.kind` へ振り分け、**対応可否の判定は純関数が spec 別に**行う。型ナローイングが `src/logic` 内で単体テスト可能になり（instanceof 実機成立は既存 `scripts/e2e` が担保）、「instanceof 配線はアダプタ側」という v0.3 の分離方針（Issue 明記）に一致する。却下「薄い `{present, value:unknown}`」＝純関数が `typeof` で判定し型情報が薄れ、kind 別テストが書きにくい。
- **未対応値＝`locked`（present を保護・決定 #3、#34 の思想を一般化）**: 生値の型が spec と不一致（boolean spec に数値等）や select 異物値は `side=undefined・locked=true` にし、ドラッグ上書きによる既存データ破壊を断つ。これは v1 の #34（非 boolean `note.*` を未分類化＋UI ロック）を軸タイプ一般に拡張したもの。**absent は保護不要**（`locked=false`）で、number/select は absent へドロップ時に代表値を書ける（AC5）。
- **tag 不在＝false 側（決定 #4）**: tag 所属は二値のため、リスト配列に対象タグが無ければ boolean の false と同じ最低象限側に確定する（`#urgent` を持たない＝urgent でない、が直感的）。tag プロパティ自体が absent の場合のみ未分類（`undefined`）。
- **象限ロジックは不変**: `classifyQuadrant`／`axisValuesForQuadrant`（`src/logic/quadrant.ts`）は #120 で変更しない。軸側の `boolean | undefined`（`AxisValues`）を安定 IF として維持し、interpretAxis の `side` をそのまま両軸ぶん流し込む（アダプタ配線は後続 L2）。

### 新規/変更モジュール（#120・実装済み）

- **新規** `src/logic/axis.ts`＝`AxisSpec`／`AxisRaw`／`AxisReading`／`AxisWritePlan` 型と `interpretAxis`／`planAxisWrite` 純関数。
...
```mermaid
flowchart TD
  V["entry.getValue(note.axis)<br/>Value | null（getValue throw は境界退避＝未分類+locked）"] --> RAW["toAxisRaw（instanceof 振り分け・AxisRaw | null）<br/>Null→absent / Bool→boolean / Number→number（Number(v.toString())・公開 API）/ String→string<br/>List・Date・Error・不明→null（未対応・正の許可リストに無い型）"]
  RAW -->|"null（未対応 Value 型＝Error/List 等）"| LK["未分類＋locked（安全側・AC2）"]
  RAW -->|"number かつ threshold=null"| V1["未分類＋locked（v1 踏襲・off-sentinel ゲート）"]
  RAW -->|"number かつ threshold あり"| NUM["interpretAxis(raw, {number, threshold})（#120・不変）<br/>→ side は interpretAxis のまま・**locked=true 強制**（1a: 書き戻し未実装）<br/>有限→象限配置(locked)・AC1/AC5 / 非有限→未分類(locked)・AC2"]
  RAW -->|"boolean / absent / string"| BOOL["interpretAxis(raw, {boolean})（#120・不変）<br/>boolean→配置(非ロック) / absent→未分類(非ロック) / string→未分類+locked（型不一致・AC2）"]
```

> spec 選択と 1a の locked 強制は `readSingleAxisReading`（`readAxis.ts`）内にインラインで持つ（専用の `specFor` 関数は無い）: raw が `number` なら `{kind:"number", threshold}`（threshold 未設定は手前で未分類+locked に分岐）、それ以外は `{kind:"boolean"}` を `interpretAxis` に渡し、number 分岐のみ返り値の `locked` を無条件 `true` に上書きする。
...
- **off-sentinel でゲート（未設定＝v1 踏襲）**: しきい値未設定の数値軸を勝手に配置せず、v1 の未分類＋locked を維持することで、v0.3 アップグレード時に既存の数値 `note.*` プロパティが不意に象限へ現れる驚きを防ぐ。設定した軸でのみ number 配置が有効（滞留 `0`＝オフ・完了 `""`＝オフと同型）。
  - **既知の割り切り（#121 レビューの question）**: グローバル既定（設定タブ）を設定済みのとき、`.base` ビュー options に空文字を置いても per-base で「数値軸オフ」にはできない（`toNumberThreshold("")→null`→`??` で設定既定へフォールバックする）。滞留しきい値が `0` を有効な off-sentinel にできる（`toThresholdDays(0)=0` で fallback しない）のと異なるが、1a はビュー options の数値 GUI が無く（`.base` 手置きのみ）・書き戻し破壊も無いため実害は無い。per-base の明示オフが要るなら、数値オプション GUI を足す段（実機スパイク後）で別 sentinel（例: `"off"` 文字列）を検討する。
